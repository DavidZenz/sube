...
hand, call the four steps directly; it is two extra lines. The manual workflow
is covered in detail in the
[modeling-and-outputs vignette](modeling-and-outputs.html); see the
[data-preparation vignette](data-preparation.html) for input contracts and the
canonical format.

These helpers work with any SUT source in the canonical long format, not just
WIOD and FIGARO. The `source` parameter selects the matching importer; once
data is in canonical `sube_suts` shape, the downstream pipeline is identical
