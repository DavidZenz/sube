...
WIOD-based. The pipeline from `build_matrices()` onward is identical for
any SUT source in canonical format - the WIOD-specific steps are the
import (section 3) and the correspondence tables (section 4).
...
- **IHS regression variants.** The 2024 paper additionally fits
  inverse-hyperbolic-sine transformed variants (linear, IHS, and mixed
  lin-IHS / IHS-lin combinations). Those are not part of `sube` v1.1;
  contact the maintainer if you need the extension.
